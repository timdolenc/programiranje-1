...
[*----------------------------------------------------------------------------*)
type field = player option (*player ali none*)
type row = field * field * field (*to je v osnovi navadn tuple, tj s tuplom skonstruiraš, sam rabiš pol tisto dvopičje dol*)
...
let winner_of_triple triple : player option =  (*The use of Some allows the function to distinguish between a triple that contains three elements of type player and a triple that contains elements of type player option.*)
  match triple with
  |(Some X, Some X, Some X) -> Some X 
  |(Some O,Some O,Some O) -> Some O
  |_-> None

let rec winner_of_list triples : player option = (*seznam triplov*)
  match triples with
  |(Some X, Some X, Some X)::xs -> Some X
  |(Some O,Some O,Some O)::xs -> Some O
  |h::tail -> winner_of_list tail
  |[] -> None
...
let place_token player grid (row_i, col_i) : state =
  ...
  let updated_grid = set_field (row_i : index) (col_i : index) x grid
